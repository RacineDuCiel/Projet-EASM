...
from src.celery_app import BACKEND_URL, WORKER_SECRET_TOKEN
...
def get_auth_headers() -> Dict[str, str]:
    """
    Returns authentication headers for worker-to-backend API calls.
    """
    return {
        "X-Worker-Token": WORKER_SECRET_TOKEN,
...
def get_session() -> requests.Session:
    ...
    if _session is None:
        ...
        _session.headers.update(get_auth_headers())
        ...
        logger.debug("Created new HTTP session with connection pooling and auth headers")
...
def get_from_backend(endpoint: str, timeout: int = HTTP_TIMEOUT) -> requests.Response:
    """
    Helper to GET data from backend with connection pooling.

    Args:
        endpoint: API endpoint path (e.g., "/scans/{id}")
        timeout: Request timeout in seconds
